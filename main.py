...
os.makedirs(ruta_jsonfolder, exist_ok=True)
...
print(text2art("AFDT", font="block"))

print("Esta herramienta realiza la siguientes funciones:\n")
        ...
        if start_date <= event_date <= end_date and event_id.text in event_ids:
            ...
            for datos in system.getchildren():
                ...
                if not fieldsSystem or all(atributo in fieldsSystem for atributo in atributos_actual):
                    atributos_filtrados = {k: v for k, v in atributos_actual.items() if k in fieldsSystem and v is not None}
...
def analizar_archivo_json():
    ...
    datos_json = []
    with open(ruta_jsonfile, "r", encoding="utf8") as f:
        for line in f:
            try:
    ...
    for entrada in datos_json:
        for campo in entrada:
            if campo.endswith("0x10") or campo.endswith("0x30"):
                fecha_hora = datetime.fromisoformat(entrada[campo])
                if fecha_hora.microsecond == 0 and entrada.get('Timestomped'):
                    if entrada not in entradas_validas:
                        entradas_validas.append(entrada)
                        log_line = " ".join([f"{k}:{entrada[k]}" for k in entrada if k in [
                            'EntryNumber', 'FileName', 'ParentPath', 'IsDirectory',
...
def recuperar_png():
    ...
    while byte:
        ...
        if found_png >= 0:
            ...
            with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                file.write(
                    '==== Archivo PNG encontrado en la localización: ' + str(hex(found_png + (size * offs))) + ' ====')
...
def recuperar_ps1():
    ...
    while byte:
        ...
        if found_ps1 >= 0:
            drec = True
            print('==== Posible archivo PS1 encontrado en la localización: ' + str(
                hex(found_ps1 + (size * offs))) + ' ====')
            with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                file.write('==== Posible archivo PS1 encontrado en la localización: ' + str(
                    hex(found_ps1 + (size * offs))) + ' ====')
            ...
            while drec:
                ...
                if byte.find(b'\n') >= 0:
                    ...
                    fileD.seek((offs + 1) * size)
                    print(
                        '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de PS1 puede ser incompleta) ====\n')
                    with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                        file.write(
                            '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de PS1 puede ser incompleta) ====\n')
                    drec = False
...
def recuperar_txt():
    ...
    while byte:
        ...
        if found_txt >= 0:
            drec = True
            print('==== Posible archivo TXT encontrado en la localización: ' + str(
                hex(found_txt + (size * offs))) + ' ====')
            with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                file.write('==== Posible archivo TXT encontrado en la localización: ' + str(
                    hex(found_txt + (size * offs))) + ' ====')
            ...
            while drec:
                ...
                if byte.find(b'\n') >= 0:
                    ...
                    fileD.seek((offs + 1) * size)
                    print(
                        '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de TXT puede ser incompleta) ====\n')
                    with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                        file.write(
                            '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de TXT puede ser incompleta) ====\n')
                    drec = False
...
def recuperar_pf():
    ...
    while byte:
        ...
        if found_prefetch >= 0:
            drec = True
            print('==== Posible archivo PREFETCH encontrado en la localización: ' + str(
                hex(found_prefetch + (size * offs))) + ' ====')
            with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                file.write('==== Posible archivo PREFETCH encontrado en la localización: ' + str(
                    hex(found_prefetch + (size * offs))) + ' ====')
            ...
            while drec:
                ...
                if byte[0] == 0:  # Assuming first byte of the next block is 0
                    ...
                    fileD.seek((offs + 1) * size)
                    print(
                        '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de PREFETCH puede ser incompleta) ====\n')
                    with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                        file.write(
                            '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de PREFETCH puede ser incompleta) ====\n')
                    drec = False
...
def recuperar_bat():
    ...
    while byte:
        ...
        if found_bat >= 0:
            drec = True
            print('==== Posible archivo BAT encontrado en la localización: ' + str(
                hex(found_bat + (size * offs))) + ' ====')
            with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                file.write('==== Posible archivo BAT encontrado en la localización: ' + str(
                    hex(found_bat + (size * offs))) + ' ====')
            ...
            while drec:
                ...
                if byte.find(b'\n') >= 0:
                    ...
                    fileD.seek((offs + 1) * size)
                    print(
                        '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de BAT puede ser incompleta) ====\n')
                    with open(os.path.join(logs_path, "afdt_temp.log"), "a") as file:
                        file.write(
                            '==== Archivo potencialmente recuperado en: ' + file_path + ' (La recuperacion de BAT puede ser incompleta) ====\n')
                    drec = False
